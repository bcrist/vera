...
fn resolve_auto_org_address(a: *Assembler, chunk: Source_File.Chunk) u32 {
        ...
        for (0.., page_sections) |page_data_handle, section_handle| {
            ...
            while (unused.findFirstSet()) |raw_unused_range_begin| {
                const address = bits.concat(.{
                    hw.addr.Offset.init(@intCast(raw_unused_range_begin)).raw(),
                    page.raw(),
                });
                const aligned_address = apply_alignment(address, alignment.modulo, alignment.offset);

                if (hw.addr.Page.init(@intCast(aligned_address >> @bitSizeOf(hw.addr.Offset))).raw() != page.raw()) break;
                const unused_range_begin = hw.addr.Offset.init(@truncate(aligned_address));
    ...
    const search_range_begin: usize = allowed_range.first >> @bitSizeOf(hw.addr.Offset);
    const search_range_end: usize = (allowed_range.first + allowed_range.len) >> @bitSizeOf(hw.addr.Offset);
    ...
    return @as(u32, @intCast(initial_page)) << @bitSizeOf(hw.addr.Offset);
}
...
fn do_chunk_layout(a: *Assembler, chunk: Source_File.Chunk, initial_address: u32) bool {
    ...
    if (chunk.section) |section_handle| {
        ...
        const initial_page = initial_address >> @bitSizeOf(hw.addr.Offset);
        const final_page = if (address == initial_address) initial_page else (address - 1) >> @bitSizeOf(hw.addr.Offset);
        ...
        for (initial_page .. final_page + 1) |page| {
            const page_data_handle = a.find_or_create_page(hw.addr.Page.init(@intCast(page)), access, section_handle);
            var page_usage = a.pages.items(.usage);
            if (page == initial_page or page == final_page) {
                const range = std.bit_set.Range{
                    .start = if (page == initial_page) hw.addr.Offset.init(@truncate(initial_address)).raw() else 0,
                    .end = if (page == final_page) hw.addr.Offset.init(@truncate(address)).raw() else Page_Data.page_size,
                };
...
pub fn populate_page_chunks(a: *Assembler, chunks: []const Source_File.Chunk) void {
    ...
    for (chunks) |chunk| {
        if (chunk.section) |section| {
            ...
            const first_page = addresses.first >> @bitSizeOf(hw.addr.Offset);
            var last_page = first_page;
            if (addresses.len > 0) {
                last_page = addresses.last() >> @bitSizeOf(hw.addr.Offset);
            }
            ...
            for (first_page .. last_page + 1) |page| {
                const page_data_handle = a.page_lookup.get(hw.addr.Page.init(@intCast(page))) orelse unreachable;
    ...
    for (0.., s.insn.items(.operation)) |insn_handle_usize, op| {
        ...
        switch (op) {
            ...
            .db, .dw, .dd => {
                ...
                var page = hw.addr.Page.init(@truncate(address >> @bitSizeOf(hw.addr.Offset)));
                const initial_offset = hw.addr.Offset.init(@truncate(address));
                ...
                while (length - written > buffer.len) {
                    ...
                    written += buffer.len;
                    page = hw.addr.Page.init(page.raw() + 1);
...
fn encode_instruction(
    ...
    const page = hw.addr.Page.init(@truncate(address >> @bitSizeOf(hw.addr.Offset)));
    ...
    if (length > buffer.len) {
        ...
        const next_page = hw.addr.Page.init(page.raw() + 1);
