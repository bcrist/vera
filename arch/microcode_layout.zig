...
pub const UC_Address = u16;
pub const UC_Continuation = u10;
pub const UC_Flags = enum { Z, N, C, V, K };
pub const UC_Flag_Set = std.EnumSet(UC_Flags);

pub const UC_Vectors = enum(UC_Address) {
    reset = 0,
    page_fault = 1,
    access_fault = 2,
    page_align_fault = 3,
...
pub fn flagPermutationIterator(flags: UC_Flag_Set) UCFlagSetPermutationIterator {
    return .{ .all = flags };
}

pub const UCFlagSetPermutationIterator = struct {
    all: UC_Flag_Set,
    permutation: UC_Flag_Set = .{},
    done: bool = false,

    pub fn next(self: *UCFlagSetPermutationIterator) ?UC_Flag_Set {
...
pub fn getCheckedFlagsForOpcode(opcode: Opcode) UC_Flag_Set {
    var flags = UC_Flag_Set{};
...
pub fn getAddressForOpcode(opcode: Opcode, flags: UC_Flag_Set) UC_Address {
    var ua: UC_Address = undefined;
    if ((opcode & 0xF000) == 0) {
...
pub fn getOpcodeGranularity(opcode: Opcode) Opcode {
    if ((opcode & 0xF000) != 0) {
        // low 4 bits (OA) are not included in the UC_Address for this opcode range
        return 16;
    } else {
        return 1;
    }
...
pub fn getOpcodeForAddress(ua: UC_Address) ?Opcode {
    if ((ua & 0x8000) == 0x8000) {
        // CNKZ or VNKZ opcode
...
pub fn isContinuationOrHandler(ua: UC_Address) bool {
    if ((ua & 0x8000) == 0x8000) {
        // CNKZ or VNKZ opcode
        return false;
    } else if ((ua & 0x4000) == 0x4000) {
...
pub fn getAddressForContinuation(n: UC_Continuation, flags: UC_Flag_Set) UC_Address {
    if ((n & 0x200) == 0) {
        // unconditional continuation cycle
...
pub fn getContinuationNumberForAddress(ua: UC_Address) ?UC_Continuation {
    if ((ua & 0x8000) == 0x8000) {
        // CNKZ or VNKZ opcode
        return null;
    } else if ((ua & 0x4000) == 0x4000) {
...
pub fn getCheckedFlagsForAddress(ua: UC_Address) UC_Flag_Set {
    var result = UC_Flag_Set{};
...
pub fn getFlagsForAddress(ua: UC_Address) UC_Flag_Set {
    var result = UC_Flag_Set{};
